...
def gen_def_matrix(x, y, k=1):
    '''
    This function generates a two dimentional polynomial design matrix
    
    representation:   Z = a1 + a2 x + a3 y + a4 x**2 + a5 xy + a6 y**2 + ...
    
    x: x-axis data, must be runned through meshgrid before hand!
    y: y-axis data, must be runned through meshgrid before hand!
    k: Denotes the order of the polynomial
    '''
    # Stretching x and y
    x = np.ravel(x)
    y = np.ravel(y)
...
def gen_sin_matrix(x, y, x0 = 0, y0 = 1953, freq = [0], spec = [0], amp = 0):
    '''
    representation:  Z = a1 + sum_i^{Nfreq} a_(1+i) sin(f_i*x*y) + a_(2+i) sin(f_i*x) + sin(freq[f]*y)
    
    freq: Frequencies obtained from "fftfreq" function
    spec: Spectrum from the data
    amp:  Here we pick the minimum amplitude
    '''
    x = np.ravel(x)
    y = np.ravel(y)
    freq = np.ravel(freq)
    ...
    xb = np.zeros((x.size, 1))
    for f in np.where(np.abs(spec) > amp)[0]:
        xb = np.c_[xb, np.sin(freq[f]*(x-x0)),np.sin(freq[f]*(y-y0))]
...
def Lasso(data_train, output_train, data_test, output_test, lam=0.0001, n_iter=10000, k=1, graph=True):
    ...
    lasso_reg = lm.Lasso(alpha=lam,max_iter=n_iter,fit_intercept=False)
